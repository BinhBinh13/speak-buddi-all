...
# Token TTL (§4.5 — access ngắn hạn, refresh dài hạn)
...
def _make_access(user: dict) -> str:
    """Phát hành access token ngắn hạn (15 phút), claim type='access'."""
    return _sign({
        "sub":   user["id"],
        "email": user["email"],
        "name":  user["name"],
...
def _make_refresh(user: dict) -> str:
    """Phát hành refresh token dài hạn (7 ngày), claim type='refresh'."""
    return _sign({
        "sub":   user["id"],
        "email": user["email"],
        "type":  "refresh",
...
def current_user(creds: HTTPAuthorizationCredentials = Depends(security)) -> dict:
    ...
    # Chỉ chấp nhận access token (hoặc token cũ không có type — tương thích ngược)
...
def _validate_email(email: str) -> bool:
    """Kiểm tra email hợp lệ: có @ và domain (SRS §5.2)."""
    return bool(_re.match(r"^[^@\s]+@[^@\s]+\.[^@\s]+$", email))

def _validate_password(pw: str) -> bool:
    """Kiểm tra password ≥ 8 ký tự và ≥ 1 chữ số (SRS §5.2)."""
...
async def register(req: RegisterRequest):
    ...
    log.info("REGISTER  new user id=%s", new_id)

    new_user = MOCK_USERS[email]
    access_token  = _make_access(new_user)
    refresh_token = _make_refresh(new_user)
...
async def login(req: LoginRequest):
    ...
    # Dùng chung 1 message cho cả "email không tồn tại" và "sai password"
    # để chống user-enumeration (AC-02-02)
    if not user or not hmac.compare_digest(pw_hash, user["password_hash"]):
        raise HTTPException(status_code=401, detail="Email hoặc mật khẩu không đúng.")

    # ── Phát access + refresh token (§4.5) ────────────────────────────────
...
async def refresh_token(req: RefreshRequest):
    """Làm mới access token bằng refresh token hợp lệ (§4.5)."""
    try:
    ...
    # Chỉ chấp nhận đúng loại refresh token
    ...
    # Tra lại user trong store để đảm bảo user vẫn tồn tại
    ...
    new_access = _make_access(user)
    log.info("REFRESH ok user=%s", user["id"])  # không log token
